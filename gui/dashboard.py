...
class HexaDashboard:
    ...
    def setup_ui(self):
        ...
        self.log = tk.Text(log_wrap, bg='#030310', fg=TEXT,
        ...
        scroll = tk.Scrollbar(log_wrap, command=self.log.yview, bg=BG3, troughcolor=BG)
        scroll.pack(side='right', fill='y')
        self.log.configure(yscrollcommand=scroll.set)

        self.log.tag_configure('s', foreground=GREEN)
        self.log.tag_configure('e', foreground=RED)
        self.log.tag_configure('i', foreground=BLUE)
        self.log.tag_configure('w', foreground=YELLOW)
        self.log.tag_configure('u', foreground=WHITE)
        self.log.tag_configure('t', foreground=DIM)
        self.log.tag_configure('n', foreground=TEXT)

        inp = ctk.CTkFrame(ctr, fg_color=BG3, corner_radius=8, border_color=BORDER, border_width=1)
        inp.pack(fill='x', padx=12, pady=(0,10))
        ctk.CTkLabel(inp, text='?', font=ctk.CTkFont(size=14), text_color=DIM).pack(side='left', padx=(10,4))
        self.cmd_in = ctk.CTkEntry(inp, placeholder_text='Type a command...',
            border_width=0, fg_color='transparent',
            font=ctk.CTkFont(size=12), text_color=WHITE)
        self.cmd_in.pack(side='left', fill='x', expand=True, pady=7)
        ...
        ctk.CTkLabel(sbar, text='Hexa - AI Desktop Assistant',
    ...
    def update_stats(self):
        try:
            ...
            self.cpu_w[0].configure(text=f'{cpu}%'); self.cpu_w[1].set(cpu/100)
            self.ram_w[0].configure(text=f'{ram.percent}%'); self.ram_w[1].set(ram.percent/100)
            self.dsk_w[0].configure(text=f'{dsk.percent}%'); self.dsk_w[1].set(dsk.percent/100)
            if bat:
                self.bat_w[0].configure(text=f'{bat.percent}%'); self.bat_w[1].set(bat.percent/100)
        except: pass
